    ...
    if not st.session_state.cart:
        ...
        if st.button(
            ...
            try:
                ...
                # Don't allow built-in preview products
                # to create broken database orders.
                if any(
                    str(item["id"]).startswith("sample")
                    for item in st.session_state.cart.values()
                ):

                    st.warning(
                        "These are sample products. "
                        "The owner needs to add/save the "
                        "products in the database before "
                        "placing an order."
                    )
