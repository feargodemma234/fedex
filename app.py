...
# HIDE ALL STREAMLIT BRANDING - CROWN + MANAGE APP
st.markdown("""
<style>
...
.store-card:hover { border-color: #22c55e; transform: translateY(-2px); }
...
if st.session_state.page == "Store":
    st.title("🛒 Quantum Store") # FEDEX STYLE
    cols = st.columns(2)
    for i, p in enumerate(products):
        with cols[i % 2]:
            st.markdown('<div class="store-card">', unsafe_allow_html=True)
            st.image(p["img"]) # REMOVED use_container_width to avoid error
