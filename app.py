...
Thank you.""")
    ...
    # CHANGED RED TEXT HERE
    st.markdown('<div class="info-box">One\'s proof of payment is made. Return to store and await confirmation email reply</div>', unsafe_allow_html=True)
    ...
    if st.button("✅ Confirm Order - I have sent the proof", use_container_width=True):
        try:
            ...
            st.error("Could not save order. Please contact support.") # HIDDEN TECHNICAL ERROR
    
    if st.button("← Back to Info"): st.session_state.page = "Checkout"; st.rerun()

# ========== CONFIRMATION ==========
elif st.session_state.page == "Confirmation":
    st.markdown(f'<div class="success-box"><h2>✅ Order Received!</h2><p>Order ID: <strong>{st.session_state.last_order}</strong></p><p>We will email you once payment is confirmed.</p></div>', unsafe_allow_html=True)
