...
.hero h2 {font-size: 20px; color: #333; font-weight: 300;}
...
if st.session_state.tab == "Foods":
    st.image("https://picsum.photos/seed/hero_food/1200/500", use_container_width=True)

    # SIGNATURE DISHES - USING PICSUM WITH SEEDS
    st.markdown("<h2 style='text-align:center; margin-top: 60px;'>Signature Dishes</h2>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    dishes = [
        ("Suya Pizza", "Spicy beef suya, onions, peppers", "$18", "https://picsum.photos/seed/suya/400/400"),
        ("Jollof Rice Bowl", "Smoky jollof, grilled chicken", "$16", "https://picsum.photos/seed/jollof/400/400"),
        ("Pepper Soup Ramen", "Goat pepper soup broth", "$20", "https://picsum.photos/seed/ramen/400/400"),
    ]
    for i, (name, desc, price, img) in enumerate(dishes):
        with [col1, col2, col3][i]:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.image(img, use_container_width=True) # THIS WILL LOAD NOW
            st.write(f"**{name}**")
            st.write(desc)
            st.write(price)
    ...
    with col2:
        st.image("https://picsum.photos/seed/build_plate/400/400", use_container_width=True)

else: # STYLE TAB
    st.image("https://picsum.photos/seed/hero_style/1200/500", use_container_width=True)
    
    # SIGNATURE PIECES
    st.markdown("<h2 style='text-align:center; margin-top: 60px;'>Signature Pieces</h2>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    clothes = [
        ("Ankara Shirt", "Premium African print", "$45", "https://picsum.photos/seed/ankara/400/400"),
        ("Street Hoodie", "CLOSE embroidered", "$60", "https://picsum.photos/seed/hoodie/400/400"),
        ("Cargo Pants", "Tactical fit", "$55", "https://picsum.photos/seed/cargo/400/400"),
    ]
    ...
    with col2:
        st.image("https://picsum.photos/seed/build_fit/400/400", use_container_width=True)
