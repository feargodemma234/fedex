...
# THIS HIDES THE CROWN AND "MADE WITH STREAMLIT" + TOP RIGHT ICON
st.markdown("""
<style>
    ...
    .viewerBadge_container__1QSob {display: none;} /* This kills the red crown */
</style>
""", unsafe_allow_html=True) # <-- fixed the : to )
...
def show_auth():
    st.title("🛒 Quantum Store") # CHANGED FROM FEDEX
        ...
        with st.form("checkout"):
            ...
            if st.form_submit_button("Place Order", use_container_width=True, type="primary"):
                if not all([name, phone, address, country, state]): st.error("Please fill all required fields")
                    ...
                    code = f"QNT-{uuid.uuid4().hex[:6].upper()}" # CHANGED FROM FEDEX
